...
#   and the number is equal to getMin()
    def initLinks(self):
        ...
        for node in range(diam):
            ...
            self.links[((node+1)%diam, node)] = []

            self.linksAmount[(node, (node+1)%diam)] = []
    ...
    def printWave(self):
        ...
        for src in range(diam):
            for dest in range(diam):
                print((src, dest), self.wave[src][dest], self.paths[(src, dest)])
            print('---------------')
...
@runTime
def main():
    radius = int(input("input the radius of ring:\n>>> "))
    ring = Torus1D(radius)
    ring.getWave()
