    ...
    def initLinks(self):
        ...
        for y in range(diam):
            for x in range(diam):
                ...
                self.links[(((x+1)%diam,y), (x,y))] = []

                self.linksAmount[((x,y), ((x+1)%diam,y))] = []

        #y axis
        for x in range(diam):
            for y in range(diam):
                self.links[((x,y), (x,(y+1)%diam))] = []
                self.links[((x,(y+1)%diam), (x,y))] = []

                self.linksAmount[((x,y), (x,(y+1)%diam))] = []
...
@runTime
def main():
    radius = int(input("input the radius of 2D torus:\n>>> "))
