...
#and the number is equal to getMin()
    def initLinks(self):
        ...
        #x axis
        for y, z, x in self.coordinate():
            self.links[((x,y,z), ((x+1)%diam,y,z))] = []
            self.links[(((x+1)%diam,y,z), (x,y,z))] = []

            self.linksAmount[((x,y,z), ((x+1)%diam,y,z))] = []

        #y axis
        for x, z, y in self.coordinate():
            self.links[((x,y,z), (x,(y+1)%diam,z))] = []
            self.links[((x,(y+1)%diam,z), (x,y,z))] = []

            self.linksAmount[((x,y,z), (x,(y+1)%diam,z))] = []
                    
        #z axis
        for x, y, z in self.coordinate():
            self.links[((x,y,z), (x,y,(z+1)%diam))] = []
            self.links[((x,y,(z+1)%diam), (x,y,z))] = []

            self.linksAmount[((x,y,z), (x,y,(z+1)%diam))] = []
...
@runTime    
def main():
    radius = int(input("input the radius of 3D torus:\n>>> "))
